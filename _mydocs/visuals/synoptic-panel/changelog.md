...
- Fix matched and unbound area colors in the Map Editor when SVG default fills could override status colors.
- Fix unavailable map references in the Map Editor ***Switch to Map*** selector.
...
- Fix a bug that caused some valid JSVG files to be rejected.
...
- Add [map navigation buttons](./options/toolbars/options.md#show-navigation) to the toolbar options.
...
- Allow areas transparency to be controlled via conditional formatting.
