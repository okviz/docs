...
When supported by a visual, you can open the Color Rules editor from a palette icon at the bottom right corner of the visual canvas or from the visual toolbar, if available.

<img src="images/color-rules-button-floating.png" height="130">
<img src="images/color-rules-button-toolbar.png" height="130">
...
3. ***Rule Target*** selects the target affected by the rule. Some visuals have a single target, while others expose multiple targets. For example, when [Card With States](../card-with-states/index.md) contains multiple cards based on different measures, the rule can be assigned to the card target it should affect.
...
When *Multiple Scales* is turned on, the editor shows one tab for each supported level. Each tab has its own *Minimum*, *Center*, *Maximum*, *Below*, and *Above* settings. This is useful when the same visual can display data at different granularities and each granularity needs its own color range. 
...
- ***Below*** and ***Above*** colors let you define the formatting used outside the configured minimum and maximum range. You can also choose a transparent color when you want values outside the range, or overlapping rules, to leave the existing visual formatting visible.
...
<img src="images/conditional-rule.png" width="500">
...
For example, in [Card With States](../card-with-states/index.md) the result will be:
...
When you need additional measures or columns to define custom rules, supported visuals may expose a ***Color Rules*** field well. 

<img src="images/color-rules-field-well.png" width="180">
