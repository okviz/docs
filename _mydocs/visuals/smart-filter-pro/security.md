...
However, it can communicate with OKVIZ services to resolve and verify the current status of your license. If [Check for updates](options/about/check-for-updates.md) is enabled, the visual can also contact its update endpoint while the report is in Editing view.
