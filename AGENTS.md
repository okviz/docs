...
3. If `local_clone_path` exists, use that local clone as the primary reference.
...
5. After a successful clone, use the new local clone as the primary reference for feature behavior, option names, field definitions, supported variants, changelog notes, and breaking changes.
...
- Automatic checkout is allowed only for missing local clones and only after explicit user approval.
...
- If the registry defines `default_branch`, the agent may clone that branch directly.
...
- Do not run `git pull`, `git fetch`, branch switches, or any other repo-updating action on an existing local clone unless the user explicitly asks for it.
